    ...
    def solve(self, N=50):
        ...
        for it in iterations:
            ...
            if best_path.distance > path.distance:
                best_path = path.copy()
            plt.cla()
            ax = path.plot('{} with p={}'.format(it, round(p,3)))
            axes.append(ax)
        self.T = T
        return best_path, axes
...
if __name__ == '__main__':
    ...
    solution, axes = solver.solve(N=1000)
    #solution.plot()
    ...
    ani = animation.ArtistAnimation(fig, axes, interval=50, blit=True,
                                    repeat_delay=1000)
